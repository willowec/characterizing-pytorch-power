...
if __name__ == "__main__":
    ...
    parser.add_argument("-is", "--input_size", type=int, default=32, help="input size")

    args = parser.parse_args()

    device = "cpu"

    model = Net(args.input_size, [args.hidden_size for i in range(args.n_layers)], 1)
    for layer in model.children():
        print(layer)

    training_X = torch.rand((args.data_size, args.input_size))
    training_y = torch.rand((args.data_size, 1))
    train_dataloader = DataLoader(TensorDataset(training_X, training_y), batch_size=args.batch_size, shuffle=True)
